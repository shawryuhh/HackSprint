...
export function CommandCenter() {
  ...
  const navigation = useRef<HTMLElement>(null); const [indicator,setIndicator] = useState({top:0,height:42});
  useLayoutEffect(() => { const root = navigation.current; if (!root) return; const measure = () => { const button = root.querySelector<HTMLElement>('[aria-current="page"]'); if (button) setIndicator({top:button.offsetTop,height:button.offsetHeight}); }; measure(); const observer = new ResizeObserver(measure); observer.observe(root); return () => observer.disconnect(); },[view,language]);
  return <div className="app-shell"><a className="skip-link" href="#command-main">{t("common.skip")}</a><aside className="sidebar"><a href="#command-main" className="brand" onClick={() => setView("overview")} aria-label="ReliefMesh"><span className="brand-mark"><i /><i /><i /><i /></span><span>relief<span>mesh</span><small>{t("app.title")}</small></span></a><div className="sidebar-section-label">{t("app.live")}</div><nav ref={navigation} aria-label={t("app.title")}><i className="nav-indicator" aria-hidden="true" style={{"--nav-top":`${indicator.top}px`,height:indicator.height} as CSSProperties} />{sections.map(({ id, Icon }) => <button className={view === id ? "nav-active" : ""} key={id} aria-label={t(`nav.${id}`)} onClick={() => setView(id)} aria-current={view === id ? "page" : undefined}><Icon size={18} /><span>{t(`nav.${id}`)}</span>{id === "incidents" && <b>{data?.metrics.activeIncidents ?? 0}</b>}</button>)}</nav><div className="sidebar-bottom"><div className="sector-card"><span className="sector-icon"><ShieldCheck size={21} /></span><strong>{t("app.demo")}</strong><p>{t("app.demoNotice")}</p></div><div className="operator"><span>OC</span><div><strong>{t("source.coordinator")}</strong><small>{t("app.subtitle")}</small></div></div></div></aside>
