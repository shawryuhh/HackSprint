...
export function IncidentDetail() {
  ...
  return <aside className="panel detail-panel" aria-label={`${selectedId} ${t("incident.structured")}`}><div className="detail-header"><div className="detail-status-row"><span className="incident-id" dir="ltr">{incident.id}</span><StatusBadge status={incident.status} /><SeverityBadge severity={incident.severity} /></div><h2>{incident.location}</h2><p><MapPin size={13} />{incident.latitude.toFixed(4)}, {incident.longitude.toFixed(4)}<span>·</span>{t(`type.${incident.type}`)}</p><div className={`next-action next-${presentation.emphasis}`} aria-live="polite"><small>{t("next.label")}</small><strong>{t(presentation.next)}</strong></div></div>
    ...
    <div data-stage="activity" className={`story-section ${story.active === "activity" ? "stage-current" : ""}`}><ActivityTimeline incidentId={incident.id} /></div>
    </div><DecisionDock key={incident.id} incident={incident} plan={plan} />
  </aside>;
}
